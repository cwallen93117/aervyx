...
test("airspace chip is above the basemap picker and exposes browser-verifiable state", () => {
  ...
  assert.match(source, /setFaaAirspaceLayerVisibility\(map, faaAirspaceEnabled\)/);
  assert.match(source, /const visibleFaaAirspaceData = faaAirspaceEnabled \? faaAirspaceData : emptyFeatureCollection\(\)/);
