...
function featureLabelCoordinates(geometry: GeoJSON.Geometry): [number, number] | null {
  const coordinates =
...
function airspaceCategoryColorExpression(): maplibregl.ExpressionSpecification {
  const entries: string[] = [];
  for (const [category, color] of Object.entries(CATEGORY_COLORS)) {
    entries.push(category, color);
  }
  return ["match", ["get", "category"], ...entries, "#64748b"] as unknown as maplibregl.ExpressionSpecification;
}

function ensureFaaAirspaceLayers(map: maplibregl.Map, isPerspective3D = false) {
  if (hasSource(map, FAA_AIRSPACE_SOURCE_ID)) {
    if (isPerspective3D) {
      removeLayerIfPresent(map, "faa-airspace-overlay-fill");
      if (!map.getLayer("faa-airspace-overlay-extrusion")) {
        safeAddLayer(map, {
          id: "faa-airspace-overlay-extrusion",
          type: "fill-extrusion",
          source: FAA_AIRSPACE_SOURCE_ID,
          paint: {
            "fill-extrusion-color": airspaceCategoryColorExpression(),
            "fill-extrusion-opacity": 0.14,
            "fill-extrusion-base": ["max", 0, ["*", ["coalesce", ["get", "lowerVal"], 0], 0.3048]],
            "fill-extrusion-height": [
              "max",
              60,
              ["*", ["coalesce", ["get", "upperVal"], ["+", ["coalesce", ["get", "lowerVal"], 0], 1500]], 0.3048],
            ],
          },
        });
      }
    } else {
      removeLayerIfPresent(map, "faa-airspace-overlay-extrusion");
      if (!map.getLayer("faa-airspace-overlay-fill")) {
        safeAddLayer(map, {
          id: "faa-airspace-overlay-fill",
          type: "fill",
          source: FAA_AIRSPACE_SOURCE_ID,
          paint: {
            "fill-color": airspaceCategoryColorExpression(),
            "fill-opacity": 0.12,
          },
        });
      }
    }
    if (!map.getLayer("faa-airspace-overlay-outline")) {
      safeAddLayer(map, {
        id: "faa-airspace-overlay-outline",
        type: "line",
        source: FAA_AIRSPACE_SOURCE_ID,
        paint: {
          "line-color": airspaceCategoryColorExpression(),
          "line-width": ["case", ["==", ["get", "category"], "TFR"], 2, 1.2],
          "line-dasharray": ["case", ["==", ["get", "category"], "TFR"], ["literal", [4, 2]], ["literal", [1, 0]]],
        },
      });
    }
  }
  if (hasSource(map, FAA_AIRSPACE_LABEL_SOURCE_ID) && !map.getLayer("faa-airspace-overlay-labels")) {
    safeAddLayer(map, {
      id: "faa-airspace-overlay-labels",
      type: "symbol",
      source: FAA_AIRSPACE_LABEL_SOURCE_ID,
      layout: {
...
function buildFaaAirspaceLabelData(data: GeoJSON.FeatureCollection): GeoJSON.FeatureCollection {
  return {
    ...
    features: data.features.flatMap((feature) => {
      ...
      const coordinates = featureLabelCoordinates(feature.geometry);
      if (!coordinates) return [];
      const properties = feature.properties as { name?: string; category?: AirspaceCategory; lowerVal?: number | null; lowerUom?: string; upperVal?: number | null; upperUom?: string } | null;
      const category = properties?.category;
      const categoryLabel = category ? CATEGORY_LABELS[category] ?? category : "Airspace";
      const lower = properties?.lowerVal ? `${properties.lowerVal} ${properties.lowerUom ?? "FT"}` : "SFC";
      const upper = properties?.upperVal ? `${properties.upperVal} ${properties.upperUom ?? "FT"}` : "UNL";
      return [{
        type: "Feature" as const,
        geometry: { type: "Point" as const, coordinates },
        properties: { label: `${properties?.name ?? categoryLabel}\n${lower} - ${upper}` },
      }];
    }),
  };
}
...
export const TaskMap = React.memo(function TaskMap({
  ...
  const faaAirspaceAbortRef = useRef<AbortController | null>(null);
  const faaAirspaceCategoriesRef = useRef<AirspaceCategory[]>(normalizeAirspaceCategories(faaAirspaceCategories));
  const faaAirspaceEnabledRef = useRef(false);
  ...
  const effectiveFaaAirspaceCategories = useMemo(
  ...
  const loadFaaAirspaceForViewport = useCallback(async (map: maplibregl.Map) => {
    const categories = faaAirspaceCategoriesRef.current;
    if (!categories.length) {
      setFaaAirspaceData(emptyFeatureCollection());
      return;
    }
    faaAirspaceAbortRef.current?.abort();
    const controller = new AbortController();
    faaAirspaceAbortRef.current = controller;
    const bounds = map.getBounds();
    try {
      const data = await fetchAirspaceCategories({
        west: bounds.getWest(),
      ...
      if (!controller.signal.aborted) {
        setFaaAirspaceData(data as GeoJSON.FeatureCollection);
      }
    } catch (error) {
  ...
  const livePilotMarkerData = useMemo(
    () =>
      effectiveLivePositions.map((position) => ({
        subjectKey: position.subjectKey ?? position.id,
        ...
        highlighted: (position.subjectKey ?? position.id) === effectiveHighlightedLiveSubjectKey,
      })),
    [effectiveAltitudeMultiplier, effectiveHighlightedLiveSubjectKey, effectiveLivePositions, units.altitude],
  );
  const livePilotLabelData = useMemo(
    ...
    [effectiveAltitudeMultiplier, effectiveHighlightedLiveSubjectKey, effectiveLiveLabelPositions, units.altitude],
  );
  ...
  const highlightedAltitudeScale = useMemo<AltitudeGradientScale | null>(() => {
    ...
    return {
      ...
      maxAltitudeM,
      baseLabel: formatAltitudeLabel(takeoffAltitudeM, units.altitude),
    ...
    if (labelData.length) {
      layers.push(
        new TextLayer({
          ...
          getSize: (item: { highlighted?: boolean }) => (item.highlighted ? 15 : 13),
          sizeUnits: "pixels",
          sizeMinPixels: 13,
          getPixelOffset: mode === "live" ? [0, -35] : [0, -20],
  ...
  }, [effectiveFaaAirspaceCategories]);

  useEffect(() => {
    faaAirspaceEnabledRef.current = faaAirspaceEnabled;
    ...
    const map = mapRef.current;
    if (map) {
      void loadFaaAirspaceForViewport(map);
    }
  }, [faaAirspaceEnabled, effectiveFaaAirspaceCategories, loadFaaAirspaceForViewport, mapReadyNonce]);
  ...
  }, [faaAirspaceData, faaAirspaceLabelData, isPerspective3D, styleGeneration]);
  ...
  const highlightedLiveSnapshot = useMemo<HighlightedTrackSnapshot | null>(() => {
    if (!effectiveHighlightedLiveSubjectKey) {
      return null;
    }
    ...
    const latestCoordinateIndex = telemetrySeries?.timestamps.length
      ...
      : highlightedFeature?.geometry.coordinates.length
        ? highlightedFeature.geometry.coordinates.length - 1
        : -1;
    const telemetryIndex = telemetrySeries && latestCoordinateIndex >= 0
      ? Math.min(latestCoordinateIndex, telemetrySeries.altitudeM.length - 1)
      : -1;
    const coordinate = highlightedFeature && latestCoordinateIndex >= 0
      ? highlightedFeature.geometry.coordinates[Math.min(latestCoordinateIndex, highlightedFeature.geometry.coordinates.length - 1)]
      : null;
    const mapCoordinate = livePosition
    ...
    return {
      pilotName: livePosition?.pilotName ?? String(highlightedFeature?.properties?.pilot_name ?? "Pilot"),
      coordinate: mapCoordinate,
      altitudeM: telemetryIndex >= 0
        ? telemetrySeries?.altitudeM[telemetryIndex] ?? livePosition?.altitudeM ?? (coordinate && coordinate.length > 2 ? Math.round(coordinate[2] ?? 0) : null)
        : livePosition?.altitudeM ?? (coordinate && coordinate.length > 2 ? Math.round(coordinate[2] ?? 0) : null),
      speedKmh: livePosition?.speedKmh ?? (telemetryIndex >= 0 ? telemetrySeries?.speedKmh[telemetryIndex] ?? null : null),
  ...
  useEffect(() => {
    highlightedTrackSnapshotRef.current = activeHighlightedTrackSnapshot;
  }, [activeHighlightedTrackSnapshot]);

  useEffect(() => {
    if (telemetryThrottleMs <= 0) {
      setDisplayedHighlightedTrackSnapshot(activeHighlightedTrackSnapshot);
      return;
    }
  ...
  }, [activeHighlightedTrackSnapshot, effectiveHighlightedLiveSubjectKey, effectiveHighlightedTrackUploadId, telemetryThrottleMs]);
  ...
  const altitudeScaleOverlay = highlightedAltitudeScale ? (
    <div className="map-altitude-scale" aria-label="Selected track altitude scale">
      <span className="map-altitude-scale-labels">
        <span className="map-altitude-scale-label map-altitude-scale-label-max">{highlightedAltitudeScale.maxLabel}</span>
        <span className="map-altitude-scale-label map-altitude-scale-label-base">{highlightedAltitudeScale.baseLabel}</span>
      </span>
