"""Generate an HTML audit report for FAI competition comparison results.

Reuses the base report generator but with an FAI-specific template.
"""
...
def generate_fai_report(
    comparisons: list[CompetitionComparison],
    output_path: Path,
) -> Path:
    """Render the FAI audit HTML report."""
    env = Environment(
    ...
    template = env.get_template("fai_report.html.j2")
